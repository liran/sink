...
func TestNativeCommandAllowsWritesAndRejectsCursorSessionState(t *testing.T) {
	tests := []struct {
		...
		{name: "drop", command: `{"drop":"products"}`},
		{name: "unknown command", command: `{"futureCommand":1}`},
		{name: "rename", command: `{"renameCollection":"catalog.products","to":"catalog.other"}`},
		{name: "map reduce", command: `{"mapReduce":"products","out":"other"}`},
		{name: "apply oplog", command: `{"applyOps":[]}`},
		{name: "drop database", command: `{"dropDatabase":1}`},
		{name: "collection conversion", command: `{"convertToCapped":"products","size":1024}`},
